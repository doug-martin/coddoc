...
addHandler(/^(\w+(?:\.\w+)*)\.prototype\.(\w+(?:\.\w+)?) *= *function/, 7, function (str) {
    var params = util.getParamList(str), activeScope = RegExp.$1, name = RegExp.$2;
    return {
        type:'function',
        isFunction:true,
        memberof:activeScope,
...
addHandler(/^(\w+(?:\.\w+)*)\.prototype\.(\w+(?:\.\w+)?) *= *([^\n;]+)/, 6, function (str) {
    var activeScope = RegExp.$1;
    return {
        type:'property',
        isFunction:false,
        memberof:activeScope,
        isStatic:activeScope ? !activeScope.match(".prototype") : false,
        isPrivate:RegExp.$2.match(/^_/) != null,
        name:RegExp.$2,
        code:RegExp.$3
    };
});
...
addHandler(/^(\w+(?:\.\w+)+) *= *([^\n;]+)/, 4, function () {
    var splitName = util.splitName(RegExp.$1), name = splitName.name, activeScope = splitName.memberof;
    return  {
        type:'property',
        isFunction:false,
        memberof:activeScope,
...
addHandler(/^var +(\w+) *= *([^\n;]+)/, 3, function(str, symbol, context){
    var activeScope = context.getActiveScopeName();
    return {
        type:'declaration',
        isFunction:false,
